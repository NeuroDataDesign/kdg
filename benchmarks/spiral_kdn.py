...
weights, bias = vanilla_nn.layers[0].get_weights()
...
#%%
fig,ax = plt.subplots()
for node in range(weights.shape[1]):
    a = weights[0, node]
    b = weights[1, node]
    c = bias[node]
    ax.axline((0, -c/b), (-c/a, 0))  

colors = sns.color_palette("Dark2", n_colors=2)
clr = [colors[i] for i in y]
ax.scatter(X[:, 0], X[:, 1], c=clr, s=50, alpha=0.2)
ax.set_xlim([-1.5, 1.5])
ax.set_ylim([-1.5, 1.5])
ax.set_aspect('equal')
plt.show() 
